...
from auth import create_access_token, hash_password, verify_password, CurrenUser
...
async def delete_user(user_id: int, current_user:CurrenUser,db: Annotated[AsyncSession, Depends(get_db)]):
    if user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,

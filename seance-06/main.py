...
def get_user(user_id: int, db : Annotated[Session, Depends(get_db)]):
    ...
    user = result.scalars().first() #return None if no match
...
def get_user_posts(user_id: int, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.User).where(models.User.id == user_id))
    user = result.scalars().first()
    #on doit vérifier que ça existe bien d'abord, ça peut return un élément nul sinon
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="User not found",
...
def update_post_partial(
    ...
    #On veut seulement update les infos données par le client (pas overwrite donnéees pas désirées)
